...
/// 가상 터미널(VT) 시퀀스를 더 높은 수준의 입력 이벤트(`Input`)로 변환
/// Parses VT sequences into input events.
pub struct Parser {
    // "브래킷 붙여넣기(bracketed paste)" 시퀀스 내부에 있는지 여부
    bracketed_paste: bool,
    // bracketed_paste 시퀀스 도중에 수신된 텍스트 데이터를 임시로 저장하는 버퍼
    bracketed_paste_buf: Vec<u8>,
    // X10 마우스 프로토콜에 따라 마우스 좌표를 기대하고 있는 상태인지 여부
    x10_mouse_want: bool,
    // X10 마우스 프로토콜에서 수신되는 세 문자의 좌표 데이터를 임시로 저장하는 버퍼
    x10_mouse_buf: [char; 3],
    // x10_mouse_buf에 현재 몇 개의 문자가 저장되었는지 추적
    x10_mouse_len: usize,
}
...
impl<'input> Stream<'_, '_, 'input> {
    // bracketed_paste 플래그가 설정되었을 때 호출되며,
    // ESC[200~와 ESC[201~ 사이의 모든 텍스트를 수집하여 Input::Paste 이벤트로 반환
    //
    ...
    // x10_mouse_want 플래그가 설정되었을 때 호출되며,
    // 들어오는 세 문자를 파싱하여 X10 마우스 프로토콜에 따른 Input::Mouse 이벤트를 생성
    //
    ...
    // CSI 시퀀스 매개변수로부터 Shift, Alt, Ctrl과
    // 같은 키보드 수정자(InputKeyMod)를 추출
